...
namespace Kiwi
{
	...
	public:
		...
		/** The function shoulds perform some stuff.
		 ...
		 @return true if the class has done something with the event otherwise false
		 */
		virtual bool receive(Focus event)
		{
			return false;
		}
	};
	...
	public:
		...
		Event(const int keycode, const long modifiers, const wchar_t character) noexcept
		: m_keycode(keycode), m_modifiers(modifiers), m_character(character)
		{
			;
		}
		
		Event(const int keycode) noexcept
		: m_keycode(keycode), m_modifiers(Nothing), m_character(0)
		{
			;
		}
		
		Event(const int keycode, const long modifiers) noexcept
